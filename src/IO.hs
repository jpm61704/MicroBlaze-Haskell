...
{-
  next_ins ← getRPC
  let load = case ins of

                 Lbui rD rA imm → Just (imm_addr, rD, ByteSize)
                 Lhu rD rA rB   → Just (addr, rD, HalfWordSize)
                 Lhui rD rA imm → Just (imm_addr, rD, HalfWordSize)
                 Lw rD rA rB    → Just (addr, rD,  WordSize)
                 Lwi rD rA rB   → Just (imm_addr, rD, WordSize)
                 _              → Nothing
                   where addr = do
                           a ← getRegister rA
                           b ← getRegister rB
                           return $ snd $ W32.add a b C
                         imm_addr = (⊥)
  let write = case ins of
                Sb rD rA rB   → Just (⊥)
                Sbi rD rA imm → Just (⊥)
                Sh rD rA rB   → Just (⊥)
                Shi rD rA imm → Just (⊥)
                ...
                Swi rD rA imm → Just (⊥)
                _             → Nothing
  return $ Out next_ins load write



makeOutbound Nothing    = (⊥)
